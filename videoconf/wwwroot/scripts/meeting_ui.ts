...
{   
    ...
    panelContainer: JQuery = null;

    panelClass: string = "videocontainer"; //every panel elements have this class
    videoElementClass: string = "video-element";

    nPanelInstanceId: number = 1;
    //increased when add new, but not decreased when remove panel

    leaveMeetingCallback: (value: void) => void;
    ...
    private registerEventHandlers()
    {
        ...
        $(`#${this.panelContainerId}`)
            ...
            .on('mouseover', `.${this.panelClass}`, function () {
                $(this).removeClass("display-video");
                $(this).addClass("display-name-on-video");
            })
            .on('mouseout', `.${this.panelClass}`, function () {
    ...
    public setAudio(index:number, status:number) {
        if (status == 0) { //disable
    ...
    public setAudioStatus(index:number) {
    ...
    public setVideo(index:number, status:number) {
        if (status == 1) {
    ...
    activateVideoPanel(index: number, status: boolean) {
        if (status) {
            $("#piece-" + index + " img.vid-back").remove();
    ...
    activateAudioButton(index: number, status: boolean)
    {
        if (!status) { //disable
            $("#piece-" + index + " img.aud-back").attr("src", "../img/mute.png");
        }
    ...
    private getVideoPanel(index: number): HTMLMediaElement {
        this.activateVideoPanel(index, true);
        return document.querySelector("#piece-" + index + " video") as HTMLMediaElement;
    }


    public getMainVideoPanel(): HTMLMediaElement {
        let videoCardViews = document.querySelectorAll(`video.${this.videoElementClass}`);
        if (videoCardViews.length <= 0) return null;
        return videoCardViews[0] as HTMLMediaElement;
    }

    public getEmptyVideoPanel(): HTMLMediaElement {
        this.addNewPanel();
        return this.getLastVideoElement();
    }

    public freeVideoPanel(videoElem: HTMLMediaElement) {
        let videoElemIndex: number = parseInt(videoElem.getAttribute("index"));
        if (videoElemIndex != NaN && videoElemIndex >= 0 && videoElemIndex < 9) {
            this.activateVideoPanel(videoElemIndex, false);
        }
    }
    ...
    private addNewPanel() {
        ...
        if (isDisableCamera) {
            ...
            videoTag = `<video id='remoteVideo_${++this.nPanelInstanceId}' class='${this.videoElementClass}'></video>`;
        } else {
            videoTag = `<video autoplay class='${this.videoElementClass}'  id="remoteVideo_${++this.nPanelInstanceId}" playsinline = "" style = "" ></video>`; //set camera parameter;
        }
        ...
        $(`#${this.panelContainerId}`).append(`<span class="${this.panelClass} display-video ` + activeSpeaker + '">'
            + videoTag + audioTag +
    ...
    private decreaseCardView() {
        var count = $(`.${this.panelClass}`).length;
        if (count <= 1) return;
        $(`#${this.panelContainer}`).children().last().remove();
        this.refreshCardViews();
    }

    private getPanelCount(): number {
        var count = $(`.${this.panelClass}`, this.panelContainer).length;
        return count;
    }

    private getLastVideoElement(): HTMLMediaElement {
        //return this.getMediaElementById(`remoteVideo_${this.videoPanel_InstanceId}`) as HTMLMediaElement;
        let videoCardViews = document.querySelectorAll(`video.${this.videoElementClass}`);
        if (videoCardViews.length <= 0) return null;
        return videoCardViews[videoCardViews.length - 1] as HTMLMediaElement;
    }

    private freeVideoElement(videoElement: HTMLMediaElement) {
