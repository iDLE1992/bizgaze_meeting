...
{   
    ...
    peerInfo: UserInfo = null;
    meetingUI: MeetingUI = null;
    ...
    constructor(channel: signalR.HubConnection, pcConfig: object, offerConstraints: object, mediaConstraints: object, sdpConstraints: object,
        sourceId: string, destId: string, initiator: boolean, stereo: boolean, localStream: MediaStream,
        myInfo: UserInfo, peerInfo: UserInfo, meetingUI: MeetingUI) {
    ...
    doCall() {
        let options:any = {};
        if (!this.myInfo.hasMediaDevice.hasCamera && this.peerInfo.hasMediaDevice.hasCamera) options.offerToReceiveVideo = true;
        if (!this.myInfo.hasMediaDevice.hasMic && this.peerInfo.hasMediaDevice.hasMic) options.offerToReceiveAudio = true;
        ...
        this.peerConnection.createOffer(options).then( (offer) => {
            ...
            this.Log("could not createOffer");
    ...
    doAnswer() {
        ...
        this.peerConnection.createAnswer(options).then((answer) => {
            ...
            this.Log("could not createAnswer");
            console.log(reason.toString());
        });
    }
